...
const RocketLaunchSequence = dynamic(() => import('@/components/RocketLaunchSequence'), {
  ...
  loading: () => (
    <div className="w-full h-full min-h-screen flex flex-col items-center justify-center bg-black font-mono">
      <div className="w-14 h-14 rounded-full border-2 border-orange-500 border-t-transparent animate-spin mb-4" />
      <div className="text-xs tracking-widest text-orange-400 uppercase animate-pulse">
    ...
    <main className="bg-black text-white min-h-[100dvh] w-full relative overflow-hidden select-none">
      ...
      <nav className="fixed top-0 left-0 w-full flex justify-between items-center z-50 bg-black/40 backdrop-blur-md border-b border-white/10 px-4 sm:px-8 py-3 pointer-events-auto">
        <div className="flex items-center gap-3">
          <Link
            href="/"
            className="text-sm sm:text-base md:text-lg tracking-widest text-white hover:text-cyan-400 transition-colors font-mono font-bold flex items-center gap-2"
          >
            <span className="w-2 h-2 rounded-full bg-orange-500 animate-pulse" />
            SATQUERY AI // LAUNCH SEQUENCE
          </Link>
          <span className="hidden md:inline-block px-2.5 py-0.5 rounded text-[10px] font-mono tracking-widest uppercase bg-orange-500/10 border border-orange-500/30 text-orange-400">

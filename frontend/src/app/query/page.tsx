                ...
                {row.map((cell, cIdx) => (
                  <td
                    ...
                    className={`py-2.5 px-4 ${
                      cIdx === row.length - 1 ? 'text-right font-mono text-white/80 font-medium' : ''
                    } ${isTotal ? 'text-white' : ''}`}
                  >
                    {renderInline(cell)}
                  </td>
  ...
  return (
    <div className={`w-full mb-2 rounded-xl border text-xs font-mono overflow-hidden ${
      ...
      <div className={`flex items-center gap-2 px-3 py-2 border-b ${
        hasIssues ? "border-amber-500/20 bg-amber-500/10" : "border-white/20 bg-white/10"
      }`}>
        <span className="text-base">{icon}</span>
        <div className="flex-1 min-w-0">
          <div className={`font-bold tracking-wider uppercase text-[10px] truncate ${
            hasIssues ? "text-amber-300" : "text-white/80"
          }`}>
      ...
      <div className="flex flex-wrap gap-3 px-3 py-2">
        ...
        {detectionCount > 0 && (
          <div className="flex items-center gap-1.5">
            ...
            <span className="px-1.5 py-0.5 rounded-md bg-white/20 text-white/80 text-[10px] font-bold">
    ...
    <main className="h-screen min-h-[100dvh] h-[100dvh] bg-black bg-cosmic-glow relative overflow-hidden flex flex-col">
      ...
      <nav className="w-full flex justify-between items-center px-4 sm:px-6 lg:px-8 py-3 sm:py-4 z-50 transition-all safe-top">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <span className="hidden md:inline-block text-[10px] px-2 py-0.5 rounded-full bg-white/15 text-white/80 font-mono font-bold tracking-wider border border-white/30">
            ISRO / SAC — PS 26167
          </span>
        </div>
        ...
        <div className="flex gap-2 sm:gap-3 items-center">
          ...
          {isAuthenticated && (
            <div className="flex items-center gap-1.5 sm:gap-2">
              <span className="text-[9px] sm:text-[10px] px-2 sm:px-2.5 py-1 rounded-full bg-white/20 text-white/80 border border-white/40 font-mono font-bold tracking-widest uppercase flex items-center gap-1">
                      ...
                      <div className="flex flex-col gap-2.5">
                        {queryCategories.map((cat, idx) => (
                          <div key={idx} className="flex flex-col gap-1">
                            <div className="text-[9px] tracking-widest text-white font-mono font-semibold uppercase">
                              {cat.category}
                            </div>
        ...
        {latestResult && (
            ...
            <div className="mb-6">
              ...
              {latestResult.pair_comparison && latestResult.pair_comparison.before_image && latestResult.pair_comparison.after_image && (
                <div className="mb-4">
                  <div className="text-[10px] font-mono tracking-wider uppercase text-white font-bold mb-2 flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-white animate-pulse"></span>
            ...
            <div className="pt-4 border-t border-[#2a2a2f] mt-6 shrink-0 flex flex-col gap-2">
              {latestResult.geojson_data && (
                <button
                  ...
                  className="w-full py-2.5 px-4 bg-white text-black font-semibold text-xs tracking-wider uppercase rounded hover:bg-white/80 transition-colors flex items-center justify-center gap-2 cursor-pointer shadow-lg"

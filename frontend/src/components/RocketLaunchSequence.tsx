...
function LaunchAnimation() {
  ...
  useFrame((state, delta) => {
    ...
    // Helper to calculate orbital trajectory based on an offset.
    // By clamping the offset, we can "freeze" spent stages in the timeline so they stop revolving.
    const getTrajectory = (clampedOffset: number, pOrbitContinuous: number, time: number) => {
      const pLiftoff = getProgress(clampedOffset, 0.05, 0.3);
      const pPitchOver = getProgress(clampedOffset, 0.15, 0.4);
      const pOrbitInsert = getProgress(clampedOffset, 0.45, 0.7);
      ...
      // Max-Q vibration (applied to the base trajectory so all attached components shake)
      if (pMaxQ > 0 && pMaxQ < 1) {
    ...
    // The satellite follows the timeline continuously (offset 0 to 1)
    ...
    if (satelliteGroupRef.current) {
      // Local separation: pushes forward (up in local space)
      satelliteGroupRef.current.position.y = 3.0 + (pSatSep * 5); 
      
      if (pSatSep > 0) {
        satelliteGroupRef.current.rotation.y += 0.5 * delta; // spin
        satelliteGroupRef.current.rotation.x = THREE.MathUtils.lerp(0, Math.PI / 4, pSatAcq);
      }
    }

    // --- STAGE 1 (Booster) ---
    // Stage 1 detaches at offset 0.3, so we freeze its orbital progression there.
    const s1Offset = Math.min(offset, 0.3);
    ...
    const pStage1Sep = getProgress(offset, 0.3, 0.6); // Time to fall
    if (stage1GroupRef.current) {
      // Local -Y moves it towards Earth center
    ...
    // --- FAIRINGS ---
    // Fairings detach at offset 0.4, freeze progression.
    const fairingOffset = Math.min(offset, 0.4);
    ...
    const pFairingSep = getProgress(offset, 0.4, 0.7);
    if (fairingLeftRef.current && fairingRightRef.current) {
      fairingLeftRef.current.position.x = -pFairingSep * 10;
      fairingLeftRef.current.position.y = 2.0 - pFairingSep * 30; // base Y was 2.0
      fairingLeftRef.current.rotation.z = pFairingSep * 3;

      fairingRightRef.current.position.x = pFairingSep * 10;
      fairingRightRef.current.position.y = 2.0 - pFairingSep * 30;
      fairingRightRef.current.rotation.z = -pFairingSep * 3;
    }

    // --- STAGE 2 (Upper Stage) ---
    // Stage 2 detaches at offset 0.7, freeze progression.
    const s2Offset = Math.min(offset, 0.7);
    ...
    const pStage2Sep = getProgress(offset, 0.7, 1.0);
    if (stage2GroupRef.current) {
      stage2GroupRef.current.position.y = 1.5 - pStage2Sep * 30; // base Y was 1.5
      stage2GroupRef.current.rotation.z = pStage2Sep * 0.5;
    }

    // CAMERA LOGIC
    // Follow the payload, then pull back to see the whole earth
    let targetCamPos = new THREE.Vector3();
    let targetLookAt = new THREE.Vector3();

    if (satelliteWrapperRef.current && satelliteGroupRef.current) {
      // We track the actual world position of the satellite group
  ...
  return (
    <group>
      ...
      <group ref={satelliteWrapperRef}>
        <group ref={satelliteGroupRef} position={[0, 3.0, 0]} scale={[0.5, 0.5, 0.5]}>
          <mesh>
      ...
      <group ref={fairingLeftWrapperRef}>
        <group ref={fairingLeftRef} position={[0, 2.0, 0]}>
      ...
      <group ref={fairingRightWrapperRef}>
        <group ref={fairingRightRef} position={[0, 2.0, 0]}>
          <mesh position={[0.3, 0.5, 0]}>

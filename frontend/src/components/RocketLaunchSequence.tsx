...
import { ScrollControls, useScroll, Html, Stars, useTexture, Trail } from '@react-three/drei';
...
function LaunchAnimation() {
  ...
  useFrame((state, delta) => {
    ...
    if (atmosphereRef.current) atmosphereRef.current.rotation.y += 0.06 * delta;

    // CAMERA LOGIC
    // Move camera to follow the action
    if (offset < 0.2) {
      state.camera.position.lerp(new THREE.Vector3(0, 2, 12), 0.05);
      state.camera.lookAt(0, 2, 0);
    } else if (offset < 0.5) {
      // Camera tilts down as rocket launches
      const t = (offset - 0.2) / 0.3;
      state.camera.position.lerp(new THREE.Vector3(0, 2 - t * 4, 12 + t * 4), 0.05);
    ...
    if (earthRef.current && atmosphereRef.current) {
      // Earth shrinks and drops significantly
      const targetScale = 1 - (liftoffProgress * 0.95); // shrinks to 5% size
      earthRef.current.scale.set(targetScale, targetScale, targetScale);
      atmosphereRef.current.scale.set(targetScale, targetScale, targetScale);
    ...
    if (exhaustRef.current && exhaustLightRef.current) {
      if (liftoffProgress > 0 && liftoffProgress < 1) {
        ...
        exhaustLightRef.current.intensity = 5 + Math.random() * 5;
      } else {
    ...
    if (rocketGroupRef.current) {
      // Rocket shakes violently during liftoff, less violently as it goes higher
      if (liftoffProgress > 0 && liftoffProgress < 1) {
    ...
    // Stage 2: Fairing Separation (offset 0.5 to 0.7)
    ...
    // Stage 3: Satellite Deployment (offset 0.7 to 0.95)
  ...
  // Reusable rocket materials
  const rocketMaterial = useMemo(() => new THREE.MeshStandardMaterial({ color: '#f0f0f0', metalness: 0.3, roughness: 0.4 }), []);
  const darkMetal = useMemo(() => new THREE.MeshStandardMaterial({ color: '#222222', metalness: 0.8, roughness: 0.2 }), []);
  const goldFoil = useMemo(() => new THREE.MeshStandardMaterial({ color: '#ffcc00', metalness: 0.6, roughness: 0.4, bumpScale: 0.05 }), []);
    ...
    <group>
      ...
      <group ref={rocketGroupRef} position={[0, -1, 0]}>
        ...
        <mesh position={[0, -3.2, 0]}>
          <cylinderGeometry args={[0.4, 0.7, 0.6, 16]} />
        ...
        <mesh ref={exhaustRef} position={[0, -4.5, 0]} visible={false}>
          ...
          <pointLight ref={exhaustLightRef} color="#ff7700" distance={20} decay={2} intensity={0} />
        </mesh>
        ...
        <group ref={fairingLeftRef} position={[0, 2, 0]}>
          <mesh position={[-0.3, 0.5, 0]}>
            {/* Half a cylinder for the base of the fairing */}
            <cylinderGeometry args={[0.6, 0.6, 2, 16, 1, false, Math.PI, Math.PI]} />
            <primitive object={rocketMaterial} attach="material" />
          </mesh>
          <mesh position={[-0.3, 2, 0]}>
            {/* Half a cone for the nose */}
            <coneGeometry args={[0.6, 1.5, 16, 1, false, Math.PI, Math.PI]} />
            <primitive object={rocketMaterial} attach="material" />
          </mesh>
        </group>
        ...
        <group ref={fairingRightRef} position={[0, 2, 0]}>
          ...
          <mesh position={[0.3, 2, 0]}>
            <coneGeometry args={[0.6, 1.5, 16, 1, false, 0, Math.PI]} />
            <primitive object={rocketMaterial} attach="material" />
          </mesh>
        </group>
        ...
        <group ref={satelliteGroupRef} position={[0, 2.5, 0]} scale={[0.5, 0.5, 0.5]}>
          ...
          <mesh ref={leftPanelRef} position={[0, 0, 0]}>
            ...
            <meshStandardMaterial color="#0a1b2c" metalness={0.9} roughness={0.1} />
          ...
          <Html distanceFactor={10} position={[0, 2.5, 0]} center>
            <div ref={htmlRef} className="flex flex-col items-center opacity-0 transition-all duration-500 pointer-events-none">
              <div className="text-[#00ffcc] font-mono text-lg md:text-2xl tracking-widest font-bold bg-black/60 px-4 py-2 border border-[#00ffcc]/50 rounded-sm shadow-[0_0_15px_rgba(0,255,204,0.3)] backdrop-blur-sm text-center">
                ORBIT ACHIEVED
              </div>
  ...
  return (
    <div className="w-full h-[100dvh] bg-[#000005] relative flex flex-col">
      ...
      <Canvas camera={{ position: [0, 2, 12], fov: 45 }}>
        ...
        <directionalLight position={[5, 10, 5]} intensity={2} color="#ffeedd" />
        <directionalLight position={[-5, 5, -5]} intensity={0.5} color="#4b70dd" />
